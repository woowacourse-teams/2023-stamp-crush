...
public class ManagerTemporaryCustomerCreateStep {
    ...
    public static Long 전화번호로_임시_고객_등록_요청하고_아이디_반환(String accessToken, TemporaryCustomerCreateRequest request) {
        ExtractableResponse<Response> response = 전화번호로_임시_고객_등록_요청(accessToken, request);
        return Long.valueOf(response.header("Location").split("/")[2]);
    }

    public static ExtractableResponse<Response> 전화번호로_임시_고객_등록_요청(String accessToken, TemporaryCustomerCreateRequest request) {
